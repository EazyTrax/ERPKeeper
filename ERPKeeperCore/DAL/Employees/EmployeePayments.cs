...
namespace KeeperCore.ERPNode.DAL.Employees
{
    public class EmployeePayments : ERPNodeDalRepository
    {
        ...
        public EmployeePayment Save(EmployeePayment employeePayment)
        {
            ...
            existPayment.PayFromAccountId = employeePayment.PayFromAccountId ?? organization.SystemAccounts.Cash.Id;
        ...
        public EmployeePayment Copy(EmployeePayment originalEmployeePayment, DateTime paydate)
        {
            var cloneEmployeePayment = erpNodeDBContext.EmployeePayments
                    .AsNoTracking()
                    .Include(p => p.EmployeePaymentLines)
            ...
            cloneEmployeePayment.EmployeePaymentLines.ToList().ForEach(ci => ci.Id = Guid.NewGuid());
            cloneEmployeePayment.EmployeePaymentPeriodId = organization.EmployeePaymentPeriods.FindByDate(paydate, true).Id;
        ...
        {
            foreach (var paymentItem in employeePayment.EmployeePaymentLines.Where(t => t.PaymentType.PayDirection == Models.Employees.PayDirection.Eanring).ToList())
            {
                trLedger.Debit(paymentItem.PaymentType.ExpenseAccount, paymentItem.Amount);
            }
        }
        ...
        {
            var paymentDeductionItems = employeePayment.EmployeePaymentLines
            ...
            {
                trLedger.Credit(paymentItem.PaymentType.ExpenseAccount, paymentItem.Amount);
            }
        }
