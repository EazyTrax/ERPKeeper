...
namespace KeeperCore.ERPNode.DAL.Transactions
{
    ...
    public class Sales : Commercials
    {
        ...
        public IQueryable<Sale> GetByStatus(TransactionStatus status)
             => this.GetQueryable()
        ...
        public int OpenCount => erpNodeDBContext.Sales.Where(b => b.Status == TransactionStatus.Open).Count();
        ...
        [NotMapped]
        public IQueryable<Sale> PaidTransactions => erpNodeDBContext.Sales.Where(s => s.Status == TransactionStatus.Paid);

        public Sale Create(Models.Profiles.Profile profile, DateTime trDate, Guid? responsibleUserId = null)
        {
            ...
            var newSale = new Sale()
            {
                ...
                CommercialItems = new HashSet<TransactionItem>(),
        ...
        public List<Transaction> FindList(Guid id) => this.erpNodeDBContext.Commercials.Where(tr => tr.Id == id).Take(1).ToList();
