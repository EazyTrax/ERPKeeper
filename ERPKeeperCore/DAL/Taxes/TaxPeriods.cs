...
namespace KeeperCore.ERPNode.DAL.Taxes
{
    public class TaxPeriods : ERPNodeDalRepository
    {
        ...
        public TaxPeriod Save(TaxPeriod salesTax)
        {
            ...
            existSalesTax.CloseToAccountId = salesTax.CloseToAccountId;
        ...
        public List<TaxPeriod> GetReadyForPost() => erpNodeDBContext.TaxPeriods
                .Where(s => s.PostStatus == LedgerPostStatus.Editable && s.CloseToAccountId != null)
                .ToList();

        public List<Transaction> GetUnassignCommercial(TaxPeriod taxPeriod) =>
                organization.CommercialTaxes.GetAssignAbleCommercial()
                .Where(com => com.TrnDate <= taxPeriod.TrnDate)
                .ToList();

        public List<Transaction> GetUnassignCommercials() => erpNodeDBContext.Commercials
        ...
        public void AutoAssignCommercial(TaxPeriod taxPeriod)
        {
            ...
            commercials.ForEach(commercial =>
            {
                ...
                commercial.TaxPeriodId = taxPeriod.Id;
            });
        ...
        public bool PostLedger(TaxPeriod taxPeriod, bool SaveImmediately = true)
        {
            ...
            if (taxPeriod.CloseToAccount.Type == AccountTypes.Asset)
                trLedger.Debit(taxPeriod.CloseToAccount, Math.Abs(taxPeriod.ClosingAmount));

            else if (taxPeriod.CloseToAccount.Type == AccountTypes.Liability)
