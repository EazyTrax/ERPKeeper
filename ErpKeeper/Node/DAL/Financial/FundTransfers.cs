...
.ToList();
        ...
        public FundTransfer Find(Guid transactionUid)
        {
            return erpNodeDBContext.FundTransfers.Find(transactionUid);
        }
        public void Save(FundTransfer transfer)
        {
            var existTransfer = erpNodeDBContext.FundTransfers.Find(transfer.Uid);
            ...
            if (existTransfer == null)
            {
                ...
                transfer.Reference = "";
                transfer.BankFeeAccountGuid = organization.SystemAccounts.BankFee.Uid;
        ...
        public FundTransfer Copy(FundTransfer originalFundTransfer, DateTime trDate)
        {
            var cloneFundTransfer = this.erpNodeDBContext.FundTransfers
                    .AsNoTracking()
                    .FirstOrDefault(x => x.Uid == originalFundTransfer.Uid);

            cloneFundTransfer.Uid = Guid.NewGuid();
        ...
        public void UnPost(FundTransfer transfer)
        {
            ...
            organization.LedgersDal.RemoveTransaction(transfer.Uid);
