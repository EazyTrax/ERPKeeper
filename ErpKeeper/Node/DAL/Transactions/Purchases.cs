...
namespace ERPKeeper.Node.DAL.Transactions
{
    public class Purchases : Commercials
    {
        ...
        {
            this.transactionType = ERPObjectType.Purchase;
        }
        ...
        public IQueryable<Purchase> GetByStatus(CommercialStatus status) => this.GetQueryable()
    ...
    .Sum(b => (Decimal?)b.Total) ?? 0;
        ...
        {
            var exPurchase = erpNodeDBContext.Purchases.Find(purchase.Uid);
            ...
            {
                exPurchase.CompanyProfile = erpNodeDBContext.Profiles.Where(p => p.IsSelfOrganization).FirstOrDefault();
                exPurchase.CompanyProfileAddress = exPurchase.CompanyProfile.PrimaryAddress;
                
                exPurchase.NoInMonth = purchase.NoInMonth;
                exPurchase.ProjectGuid = purchase.ProjectGuid;
                exPurchase.TrnDate = purchase.TrnDate;
                exPurchase.Reference = purchase.Reference;
                exPurchase.Memo = purchase.Memo;
                exPurchase.PaymentTermGuid = purchase.PaymentTermGuid;
                exPurchase.ShippingTermGuid = purchase.ShippingTermGuid;
        ...
        public Purchase Copy(Purchase originalPurchase, DateTime trDate)
        {
            var clonePurchase = this.erpNodeDBContext.Purchases
                    ...
                    .FirstOrDefault(x => x.Uid == originalPurchase.Uid);

            if (clonePurchase == null)
                throw new Exception("Copy Fail, Transaction not found");
            else
            {
                clonePurchase.Uid = Guid.NewGuid();
                ...
                clonePurchase.CommercialItems.ToList().ForEach(ci => ci.Uid = Guid.NewGuid());
        ...
        public void UnPost(Purchase purchase)
        {
            ...
            organization.LedgersDal.RemoveTransaction(purchase.Uid);
