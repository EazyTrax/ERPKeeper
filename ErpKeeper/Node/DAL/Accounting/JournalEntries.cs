...
namespace ERPKeeper.Node.DAL.Accounting
{
    ...
    public class JournalEntries : ERPNodeDalRepository
    {
        ...
        {
            var existJourmalEntry = this.Find(journalEntry.Uid);
            ...
            existJourmalEntry.UpdateBalance();
        ...
        {
            var jourmalEntry = erpNodeDBContext.JournalEntries.Create();

            jourmalEntry.Uid = Guid.NewGuid();
            jourmalEntry.JournalEntryTypeGuid = entryTypeGuid;
            jourmalEntry.TrnDate = DateTime.Now;
            jourmalEntry.No = this.GetNextNumber();
        ...
        {
            organization.LedgersDal.RemoveTransaction(journalEntry.Uid);
